...
Weixin.init = function () {
    ...
    return $.getJSON("http://r.intonead.com/wx/jsticket/toysrus?callback=?").then(function (res) {
        if (!res.success) {
...
//开启查找ibeacon
Weixin.startSearchBeacons = function(callback) {
...
//关闭查找ibeacon
Weixin.stopSearchBeacons = function(callback) {
...
var beaconListeners = [];
//监听查找ibeacon
Weixin.initBeaconListener = function() {
    wx.onSearchBeacons({
        complete: function(argv) {
            for(var i = 0; i < beaconListeners.length; i++) {
                if(typeof beaconListeners[i] == 'function') {
                    beaconListeners[i](argv);
                }
            }
        }
    });
...
//添加监听ibeacon
Weixin.addBeaconListener = function(callback) {
    var index = beaconListeners.indexOf(callback);
    if(!~index) {
        beaconListeners.push(callback);
    }
};
//删除监听ibeacon
Weixin.removeBeaconListener = function(callback) {
    var index = beaconListeners.indexOf(callback);
    if(~index) {
        beaconListeners.splice(index, 1);
    } else {
        return false;
    }
};
