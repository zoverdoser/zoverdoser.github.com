...
/***/ function(module, exports, __webpack_require__) {
	...
	Loading.show();
	$.getJSON(serverPath + 'lego1208/index/prize').done(function (res) {
	    Loading.hide();
	    ...
	    console.log(res);
	    if (res.success) {
	        var prizes = res.result.prize;
	        for (var i = 0; i < prizes.length; i++) {
	            var prizeID = prizes[i].luckydraw_gift_id;
	            if (prizeID == '96') {
	                //拼砌包
	...
	$(".prizeTicket.block").on("click", function () {
	    if ($(this).hasClass('on') && !$(this).hasClass('got')) {
	        var isFilled = Number($(this).attr('filled'));
	        if (isFilled) {
	            $(".pop.getBlock").fadeIn();
	        } else {
	...
	$(".getBtn").on("click", function () {
	    var shop = $("#shopIpt").val();
	    if (shop.length) {
	        Loading.show();
	        $.getJSON(serverPath + 'lego1208/index/prize-get', { win_code: prizeCode, shopID: shop }).done(function (res) {
	            Loading.hide();
	            if (res.success) {
	                $(".prizeTicket").eq(2).addClass('got');
